...
int pivot(qvector &v, int l, int r);
...
monotonic(const qvector &v)
{
    ...
    for (unsigned int i = 1; i < size; ++i)
    {
        if (v[i] >= monotonic)  // >= allows for many of same value,
            monotonic = v[i];   // i.e. not "strictly" monotonic
        else
            return false;
    }
    return true;
...
)
{
    ...
    case AUTOMATIC:
    {
        ...
        unsigned int iterations = 10;   // default iterations count
        if (argc > 1)
        ...
        for (unsigned int i = 0; i < iterations; ++i)
        {
            ...
            unsigned int length = static_cast<unsigned int>(drand48() * MAX_SIZE);
            if (length == 0)
            {
